...
/*Perform the dili's keygen function out of TSX
* success: 1, fail :0
*/
int dili_normal_keygen(KEYGEN_Para *genpara_u){
	KEYGEN_Para *genpara_k = NULL;
	genpara_k = vmalloc(sizeof(KEYGEN_Para));
	if(genpara_k == NULL)
		return 0;

    if(crypto_sign_keypair(genpara_k->pk, genpara_k->sk) != 0)
    {
    	vfree(genpara_k);
		return 0;
    }

    if(copy_to_user((unsigned char *)genpara_u, (unsigned int *)genpara_k, sizeof(KEYGEN_Para)))
	{
		vfree(genpara_k);
		return 0;
	}
...
/*Perform the dili's keygen function out of TSX, but we will encrypt sk
* success: 1, fail :0
*/
int dili_safe_keygen(KEYGEN_Para *genpara_u){
	KEYGEN_Para *genpara_k = NULL;
...
{	
	switch(cmd){
		...
		case DILITHIUM_KEYGEN:{
			KEYGEN_Para *keygenpara_u = (KEYGEN_Para *)arg;
			if(dili_normal_keygen(keygenpara_u)!=1)
				goto err;
			break;
		}

		case DILITHIUM_SAFE_KEYGEN:{
			KEYGEN_Para *keygenpara_u = (KEYGEN_Para *)arg;
			if(dili_safe_keygen(keygenpara_u)!=1)
				goto err;
			break;
		}
