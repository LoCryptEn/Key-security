...
static uint32 ror(uint32 n, uint32 k)
{
	return (n >> k) | (n << (32 - k));
}
...
static void sha256_transform(SHA256_CTX *s, const uint8 *buf)
{
	uint32 t1, t2, a, b, c, d, e, f, g, h, m[64];
	uint32 i, j;
...
void sha256_final(SHA256_CTX *s, uint8 *md)
{
	uint32 r = s->len % SHA256_BLOCKLEN;
	int i;
...
void sha256_update(SHA256_CTX *s, const uint8 *m, uint32 len)
{
	const uint8 *p = m;
	uint32 r = s->len % SHA256_BLOCKLEN;
...
void sha256(unsigned char* dest, const uint8 *msg, uint32 mlen)
{
		uint8 md[SHA256_DIGESTLEN] = {0};
		SHA256_CTX sha;
		sha256_init(&sha);
...
void hmac_sha256(unsigned char* dest, const uint8 *key, uint32 klen, const uint8 *msg, uint32 mlen)
{
		uint8 md[SHA256_DIGESTLEN] = {0};
...
void pbkdf2_sha256(const uint8 *key, uint32 keylen, const uint8 *salt, uint32 saltlen, uint32 rounds,
    uint8 *dk, uint32 dklen)
{
	...
	uint32 hlen = SHA256_DIGESTLEN;
	uint32 l = dklen / hlen + ((dklen % hlen) ? 1 : 0);
	uint32 r = dklen - (l - 1) * hlen;
