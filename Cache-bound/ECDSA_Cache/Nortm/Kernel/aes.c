...
int AESEncryptECB_Reg(AES_CYPHER_T mode,uint8_t *Plain, int PlainLength, uint8_t *Cipher, int CipherLength, uint8_t *Key)
{
    ...
    {
        if(Key==NULL)
            aes_enc_master(PlainAndPad+i*16, Cipher+i*16);
        else
...
int AESDecryptECB_Reg(AES_CYPHER_T mode, uint8_t *Cipher, int CipherLength, uint8_t *Plain, int PlainLength, uint8_t *Key)
{
    uint8_t PlainAndPad[CipherLength];
    int i;
    ...
    {
        if(Key==NULL)
            aes_dec_master(Cipher+i*16,PlainAndPad+i*16);
        else
...
int AESEncryptWithMode(uint8_t *Plain, int PlainLength, uint8_t *Cipher, int CipherLength,unsigned char *IV, int mode, uint8_t *Key)
{
    if(mode == ECB)
    {
    	...
    	return AESEncryptECB_Reg(AES_CYPHER_128,Plain,PlainLength,Cipher,CipherLength,Key);
        
    }    
    else if(mode == CBC)
    {
        //return AESEncryptCBC(AES_CYPHER_128,Plain,PlainLength,Cipher,CipherLength,IV,Key);
    	return AESEncryptCBC_Reg(AES_CYPHER_128,Plain,PlainLength,Cipher,CipherLength,IV,Key);
    }
    else
        return 0;
}
...
int AESDecryptWithMode(uint8_t *Cipher, int CipherLength, uint8_t *Plain, int PlainLength, unsigned char *IV, int mode, uint8_t *Key)
{
    if(mode == ECB)
    {
    	...
    	return AESDecryptECB_Reg(AES_CYPHER_128,Cipher,CipherLength,Plain,PlainLength,Key);
    }
    else if(mode == CBC)
    {
        //return AESDecryptCBC(AES_CYPHER_128,Cipher,CipherLength,Plain,PlainLength,IV,Key);
    	return AESDecryptCBC_Reg(AES_CYPHER_128,Cipher,CipherLength,Plain,PlainLength,IV,Key);
    }
    else
        return 0;
}
...
void aes_cypher_128_test(void)
{
 ...
 //   aes_encrypt(AES_CYPHER_128, buf, sizeof(buf), key);
    ...
    AESEncryptCBC_Reg(AES_CYPHER_128,buf,32,c,32,iv,key);
    //printhex(c,32);

//    aes_decrypt(AES_CYPHER_128, buf, sizeof(buf), key);
    AESDecryptCBC_Reg(AES_CYPHER_128,buf,32,c,32,iv,key);
    //printhex(buf,32);
