...
{
    uint64 x[NUM_ECC_DIGITS];
    uint64 y[NUM_ECC_DIGITS];
} EccPoint;
...
static uint64 curve_p[NUM_ECC_DIGITS] = CONCAT(Curve_P_, ECC_CURVE);
//static uint64 curve_b[NUM_ECC_DIGITS] = CONCAT(Curve_B_, ECC_CURVE);
static EccPoint curve_G = CONCAT(Curve_G_, ECC_CURVE);
static uint64 curve_n[NUM_ECC_DIGITS] = CONCAT(Curve_N_, ECC_CURVE);
...
static int getRandomNumber(uint64 *p_vli)
{
    HCRYPTPROV l_prov;
...
// static int getRandomNumber(uint64 *p_vli)
// {
//     int l_fd = filp_open("/dev/urandom", O_RDONLY | O_CLOEXEC);
//     if(l_fd == -1)
//     {
//         l_fd = filp_open("/dev/random", O_RDONLY | O_CLOEXEC);
//         if(l_fd == -1)
//         {
//             return 0;
//         }
...
static void vli_clear(uint64 *p_vli)
{
    uint i;
...
static int vli_isZero(uint64 *p_vli)
{
    uint i;
    for(i = 0; i < NUM_ECC_DIGITS; ++i)
    {
...
static uint64 vli_testBit(uint64 *p_vli, uint p_bit)
{
    return (p_vli[p_bit/64] & ((uint64)1 << (p_bit % 64)));
}

/* Counts the number of 64-bit "digits" in p_vli. */
static uint vli_numDigits(uint64 *p_vli)
{
    int i;
...
static uint vli_numBits(uint64 *p_vli)
{
    uint i;
    uint64 l_digit;
...
/* Sets p_dest = p_src. */
static void vli_set(uint64 *p_dest, uint64 *p_src)
{
    uint i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
...
static int vli_cmp(uint64 *p_left, uint64 *p_right)
{
    int i;
...
static uint64 vli_lshift(uint64 *p_result, uint64 *p_in, uint p_shift)
{
    uint64 l_carry = 0;
    uint i;
    for(i = 0; i < NUM_ECC_DIGITS; ++i)
    {
        uint64 l_temp = p_in[i];
...
static void vli_rshift1(uint64 *p_vli)
{
    uint64 *l_end = p_vli;
    uint64 l_carry = 0;
...
static uint64 vli_add(uint64 *p_result, uint64 *p_left, uint64 *p_right)
{
    uint64 l_carry = 0;
    uint i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
        uint64 l_sum = p_left[i] + p_right[i] + l_carry;
        if(l_sum != p_left[i])
        {
...
static uint64 vli_sub(uint64 *p_result, uint64 *p_left, uint64 *p_right)
{
    uint64 l_borrow = 0;
    uint i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
        uint64 l_diff = p_left[i] - p_right[i] - l_borrow;
        if(l_diff != p_left[i])
        {
...
static void vli_mult(uint64 *p_result, uint64 *p_left, uint64 *p_right)
{
    uint128_t r01 = 0;
    uint64 r2 = 0;
    ...
    for(k=0; k < NUM_ECC_DIGITS*2 - 1; ++k)
    {
        ...
        p_result[k] = (uint64)r01;
        r01 = (r01 >> 64) | (((uint128_t)r2) << 64);
        r2 = 0;
    }
    
    p_result[NUM_ECC_DIGITS*2 - 1] = (uint64)r01;
}

/* Computes p_result = p_left^2. */
static void vli_square(uint64 *p_result, uint64 *p_left)
{
    uint128_t r01 = 0;
    uint64 r2 = 0;
...
static uint128_t mul_64_64(uint64 p_left, uint64 p_right)
{
    uint128_t l_result;
    
    uint64 a0 = p_left & 0xffffffffull;
    uint64 a1 = p_left >> 32;
    uint64 b0 = p_right & 0xffffffffull;
    uint64 b1 = p_right >> 32;
...
static void vli_modAdd(uint64 *p_result, uint64 *p_left, uint64 *p_right, uint64 *p_mod)
{
    uint64 l_carry = vli_add(p_result, p_left, p_right);
...
/* Computes p_result = (p_left - p_right) % p_mod.
   Assumes that p_left < p_mod and p_right < p_mod, p_result != p_mod. */
static void vli_modSub(uint64 *p_result, uint64 *p_left, uint64 *p_right, uint64 *p_mod)
{
    uint64 l_borrow = vli_sub(p_result, p_left, p_right);
    if(l_borrow)
...
static void vli_mmod_fast(uint64 *p_result, uint64 *p_product)
{
    uint64 l_tmp[NUM_ECC_DIGITS];
    int l_carry;
...
static void omega_mult(uint64 *p_result, uint64 *p_right)
{
    uint64 l_tmp[NUM_ECC_DIGITS];
...
static void vli_mmod_fast(uint64 *p_result, uint64 *p_product)
{
    ...
    while(!vli_isZero(p_product + NUM_ECC_DIGITS)) /* While c1 != 0 */
    {
        ...
        {
            uint64 l_sum = p_product[i] + l_tmp[i] + l_carry;
...
static void vli_modMult_fast(uint64 *p_result, uint64 *p_left, uint64 *p_right)
{
    uint64 l_product[2 * NUM_ECC_DIGITS];
    vli_mult(l_product, p_left, p_right);
    vli_mmod_fast(p_result, l_product);
}

/* Computes p_result = p_left^2 % curve_p. */
static void vli_modSquare_fast(uint64 *p_result, uint64 *p_left)
{
    uint64 l_product[2 * NUM_ECC_DIGITS];
...
static void vli_modInv(uint64 *p_result, uint64 *p_input, uint64 *p_mod)
{
    uint64 a[NUM_ECC_DIGITS], b[NUM_ECC_DIGITS], u[NUM_ECC_DIGITS], v[NUM_ECC_DIGITS];
    uint64 l_carry;
    int l_cmpResult;
...
/* Double in place */
static void EccPoint_double_jacobian(uint64 *X1, uint64 *Y1, uint64 *Z1)
{
    /* t1 = X, t2 = Y, t3 = Z */
    uint64 t4[NUM_ECC_DIGITS];
    uint64 t5[NUM_ECC_DIGITS];
    ...
    if(vli_testBit(X1, 0))
    {
        uint64 l_carry = vli_add(X1, X1, curve_p);
        vli_rshift1(X1);
...
static void apply_z(uint64 *X1, uint64 *Y1, uint64 *Z)
{
    uint64 t1[NUM_ECC_DIGITS];
...
/* P = (x1, y1) => 2P, (x2, y2) => P' */
static void XYcZ_initial_double(uint64 *X1, uint64 *Y1, uint64 *X2, uint64 *Y2, uint64 *p_initialZ)
{
    uint64 z[NUM_ECC_DIGITS];
...
/* Input P = (x1, y1, Z), Q = (x2, y2, Z)
   ...
   or P => P', Q => P + Q
*/
static void XYcZ_add(uint64 *X1, uint64 *Y1, uint64 *X2, uint64 *Y2)
{
...
/* Input P = (x1, y1, Z), Q = (x2, y2, Z)
   ...
   or P => P - Q, Q => P + Q
*/
static void XYcZ_addC(uint64 *X1, uint64 *Y1, uint64 *X2, uint64 *Y2)
{
    /* t1 = X1, t2 = Y1, t3 = X2, t4 = Y2 */
    uint64 t5[NUM_ECC_DIGITS];
    uint64 t6[NUM_ECC_DIGITS];
    uint64 t7[NUM_ECC_DIGITS];
...
static void EccPoint_mult(EccPoint *p_result, EccPoint *p_point, uint64 *p_scalar, uint64 *p_initialZ)
{
    /* R0 and R1 */
    uint64 Rx[2][NUM_ECC_DIGITS];
    uint64 Ry[2][NUM_ECC_DIGITS];
    uint64 z[NUM_ECC_DIGITS];
...
static void ecc_bytes2native(uint64 p_native[NUM_ECC_DIGITS], const uint8 p_bytes[ECC_BYTES])
{
    unsigned i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
        const uint8 *p_digit = p_bytes + 8 * (NUM_ECC_DIGITS - 1 - i);
        p_native[i] = ((uint64)p_digit[0] << 56) | ((uint64)p_digit[1] << 48) | ((uint64)p_digit[2] << 40) | ((uint64)p_digit[3] << 32) |
            ((uint64)p_digit[4] << 24) | ((uint64)p_digit[5] << 16) | ((uint64)p_digit[6] << 8) | (uint64)p_digit[7];
    }
}

static void ecc_native2bytes(uint8 p_bytes[ECC_BYTES], const uint64 p_native[NUM_ECC_DIGITS])
{
    unsigned i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
        uint8 *p_digit = p_bytes + 8 * (NUM_ECC_DIGITS - 1 - i);
...
/*
static void mod_sqrt(uint64 a[NUM_ECC_DIGITS])
{
    unsigned i;
    uint64 p1[NUM_ECC_DIGITS] = {1};
    uint64 l_result[NUM_ECC_DIGITS] = {1};
...
static void ecc_point_decompress(EccPoint *p_point, const uint8 p_compressed[ECC_BYTES * 2])
{
    ecc_bytes2native(p_point->x, p_compressed);
    ecc_bytes2native(p_point->y, p_compressed + ECC_BYTES);
#if 0
    uint64 _3[NUM_ECC_DIGITS] = {3}; /* -a = 3 */
...
{
    uint64 l_private[NUM_ECC_DIGITS];
    EccPoint l_public;
    unsigned l_tries = 0;
    //uint32 i = 0;
...
int ecdh_shared_secret(const uint8 p_publicKey[ECC_BYTES+1], const uint8 p_privateKey[ECC_BYTES], uint8 p_secret[ECC_BYTES])
{
    ...
    EccPoint l_product;
    uint64 l_private[NUM_ECC_DIGITS];
    uint64 l_random[NUM_ECC_DIGITS];
...
/* Computes p_result = (p_left * p_right) % p_mod. */
static void vli_modMult(uint64 *p_result, uint64 *p_left, uint64 *p_right, uint64 *p_mod)
{
    uint64 l_product[2 * NUM_ECC_DIGITS];
    uint64 l_modMultiple[2 * NUM_ECC_DIGITS];
    ...
    while(l_productBits > NUM_ECC_DIGITS * 64 || vli_cmp(l_modMultiple, p_mod) >= 0)
    {
        ...
        uint64 l_carry = (l_modMultiple[NUM_ECC_DIGITS] & 0x01) << 63;
...
{
    uint64 k[NUM_ECC_DIGITS];
    uint64 l_tmp[NUM_ECC_DIGITS];
    uint64 l_s[NUM_ECC_DIGITS];
    EccPoint p;
    unsigned l_tries = 0;
...
int ecdsa_sign_safe(uint8 p_privateKey[ECC_BYTES], const uint8 p_hash[ECC_BYTES], uint8 p_signature[ECC_BYTES*2])
{
	...
	if(AESDecryptWithMode(p_privateKey, 32, p_privateKey, 32, NULL, ECB, NULL) == 0){
		return 0;
	}
    ...
    if(AESEncryptWithMode(p_privateKey, 32, p_privateKey, 32, NULL, ECB, NULL) == 0){
		return 0;
	}
...
{
    uint64 u1[NUM_ECC_DIGITS], u2[NUM_ECC_DIGITS];
    uint64 z[NUM_ECC_DIGITS];
    EccPoint l_public, l_sum;
    uint64 rx[NUM_ECC_DIGITS];
    uint64 ry[NUM_ECC_DIGITS];
    uint64 tx[NUM_ECC_DIGITS];
    uint64 ty[NUM_ECC_DIGITS];
    uint64 tz[NUM_ECC_DIGITS];
    uint64 l_r[NUM_ECC_DIGITS], l_s[NUM_ECC_DIGITS];
...
int SignMessage(uint8 p_privateKey[ECC_BYTES], uint8 p_msg[MSG_LEN], uint8 p_signature[ECC_BYTES*2], int p_len){
    unsigned char digest[256];
	//e = sha256(msg)
	sha256(digest,p_msg,p_len);

    return ecdsa_sign(p_privateKey, digest, p_signature);
}
...
int SignMessageSafe(uint8 p_privateKey[ECC_BYTES], uint8 p_msg[MSG_LEN], uint8 p_signature[ECC_BYTES*2], int p_len){
	...
	return ecdsa_sign_safe(p_privateKey, digest, p_signature);
}
...
int VerifySign(const uint8 p_publicKey[ECC_BYTES * 2], uint8 p_msg[MSG_LEN], const uint8 p_signature[ECC_BYTES*2],int p_len){
	...
	//e = sha256(msg)
	sha256(digest,p_msg,p_len);
    return ecdsa_verify(p_publicKey, digest, p_signature);
}